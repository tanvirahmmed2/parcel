...
export async function PATCH(req, { params }) {
  try {
    ...
    if (parcel.status !== "In Transit" && parcel.status !== "Picked Up") {
      return new NextResponse("Invalid status for receiving", { status: 400 });
    }
    ...
    parcel.history.push({
      status: "Hub Received",
      message: `Parcel received at sorting facility by Hub Manager: ${session.name}`,
    ...
    return NextResponse.json({ 
      success: true, 
      message: "Parcel received at Hub facility",
      parcel 
    });
  } catch (error) {
